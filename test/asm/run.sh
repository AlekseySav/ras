...
mem_nasm="0xfff/200/0x123/0xffff/-1/-32768/-32769/0x10/bx+si/bx+di/bp+si/bp+di/si/di/bx/bx+0x12/bx+si+1/eax/ecx/edx/ebx/esi/edi/esp/ebp/eax+eax/100000+eax+ecx/13+eax+edx/eax+ebx/edx+esi/esp+ebp/edx+ebp*2/esi+edi*8/1000+eax/dword 0x12345678/es:0/cs:di/fs:0xffff"
mem_ras="0xfff/200/0x123/0xffff/-1/-32768/-32769/0x10/(bx,si)/(bx,di)/(bp,si)/(bp,di)/(si)/(di)/(bx)/0x12(bx)/1(bx,si)/(eax)/(ecx)/(edx)/(ebx)/(esi)/(edi)/(esp)/(ebp)/(eax,eax)/100000(eax,ecx)/13(eax,edx)/(eax,ebx)/(edx,esi,0)/(esp,ebp)/(edx,ebp,1)/(esi,edi,3)/1000(eax)/0x12345678/es:0/cs:(di)/fs:0xffff"
db_nasm="\$-125/\$/\$+129"
db_ras=".-125/./.+129"
