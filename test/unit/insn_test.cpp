...
"/*.align*/ 4;"
    ...
    update_insn(set_dot);
    ASSERT_EQ(state::dot().value, 3);
    update_insn(a2);
    state::dot().value += a2->size;
    ASSERT_EQ(state::dot().value, 4);
    update_insn(a8);
    state::dot().value += a8->size;
    ASSERT_EQ(state::dot().value, 8);
    update_insn(a4);
    state::dot().value += a4->size;
    ASSERT_EQ(state::dot().value, 8);
    flush_insn(byte, out);
    flush_insn(word, out);
